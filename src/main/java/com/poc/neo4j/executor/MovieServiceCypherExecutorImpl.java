 ...
 * @since 30.05.12
 */
class MovieServiceCypherExecutorImpl {

    private final CypherExecutor cypher;

    public MovieServiceCypherExecutorImpl(String uri) {
    ...
    public void createMovie(String title, String released, String tagline) {
        if (StringUtils.isNotEmpty(title) &&
                StringUtils.isNotEmpty(released) &&
                StringUtils.isNotEmpty(tagline)
                ) {
            cypher.query(
                    "CREATE (a:Movie {title:{title}, released:{released}, tagline:{tagline}})",
                    map("title", title, "released", released, "tagline", tagline)
            );
        }
    }
